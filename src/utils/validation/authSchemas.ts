...
import { ILoginForm, IRegisterForm } from '@/interfaces/auth.interfaces';
import * as Yup from 'yup';

export const registerSchema: Yup.ObjectSchema<IRegisterForm> =
	Yup.object().shape({
		name: Yup.string()
		...
		password: Yup.string()
			...
			.matches(
				regexpConstants.PASSWORD,
				'Пароль повинен містити цифри та латинські літери',
			),
...
export const loginSchema: Yup.ObjectSchema<ILoginForm> = Yup.object().shape({
	login: Yup.string().required(
		'Вкажіть пошту або телефон в форматі: +380ХХХХХХХХХ',
	),
	password: Yup.string()
		.required('Вкажіть пароль')
		.min(6, 'Не менше 6 символів')
...
export const requestSchema: Yup.ObjectSchema<{ email: string }> =
	Yup.object().shape({
		email: Yup.string()
			.required('Вкажіть пошту')
			.matches(regexpConstants.EMAIL, 'Пошту вказано невірно'),
